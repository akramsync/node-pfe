...
router.post("/", async (req, res) => {
  try {
    ...
    res.status(201).json({
      ...
      message: "Temperature and humidity entry created successfully",
    });
  } catch (err) {
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const updatedEntry = await TemperatureHumidity.findByIdAndUpdate(
      id,
    ...
    if (!updatedEntry) {
      return res
        .status(404)
        .json({ error: "Temperature and humidity entry not found" });
    }

    res.json({
      message: "Temperature and humidity entry updated successfully",
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    res.json({
      message: "Temperature and humidity entry deleted successfully",
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
